  ...
  const debugLog = (...args) => {
    if (config.debugLogs === true || typeof config.debugLogs === "object") {
      ...
      if (typeof args[0] === "object" && "method" in args[0]) {
        ...
        env.logger.info(`[${config.adapterName}]`, ...args);
      } else {
        env.logger.info(`[${config.adapterName}]`, ...args);
      }
    }
...
function formatTransactionId(transactionId2) {
  if (env.getColorDepth() < 8) {
    return `#${transactionId2}`;
  }
  return `${env.TTY_COLORS.fg.magenta}#${transactionId2}${env.TTY_COLORS.reset}`;
}
function formatStep(step, total) {
  return `${env.TTY_COLORS.bg.black}${env.TTY_COLORS.fg.yellow}[${step}/${total}]${env.TTY_COLORS.reset}`;
}
function formatMethod(method) {
  return `${env.TTY_COLORS.bright}${method}${env.TTY_COLORS.reset}`;
}
function formatAction(action) {
  return `${env.TTY_COLORS.dim}(${action})${env.TTY_COLORS.reset}`;
}
